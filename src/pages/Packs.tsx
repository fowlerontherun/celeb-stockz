...
import { EyeOff, LockKeyhole, PackageOpen, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen bg-[#120b20] px-5 py-8 text-[#fff8f2] sm:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <header className="mt-8 rounded-[30px] border border-[#c99bff]/30 bg-[#211230] p-6 sm:p-8">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-6 text-[#c9b8d4]">Standard celebrities remain open to every player. Upcoming packs stay classified until officially announced and scheduled for launch.</p>
        </header>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div
                      className={`grid h-11 w-11 place-items-center rounded-2xl ${
                        isClassified
                          ? "bg-white/5 text-[#a99ab7]"
                          : "bg-[#7c3aed] text-white"
                      }`}
                    >
                      {isClassified ? <EyeOff size={20} /> : <PackageOpen size={21} />}
                    </div>
                    <span
                  ...
                  <h2 className="font-display mt-1 text-2xl font-black">
                    {isClassified ? (
                      <span className="italic tracking-wide text-[#a594b4]">
                        Classified
                      </span>
                    ) : (
                      pack.name
                  ...
                  <div className="mt-5 flex items-center justify-between rounded-xl bg-white/[.05] px-3 py-3">
                    <span className="text-sm font-bold">
                      {pack.isAvailable
                        ...
                        ? new Intl.DateTimeFormat([], { dateStyle: "medium" }).format(new Date(pack.availableAt))
                        : "To be announced"}
                    </span>
                    {!pack.unlocked && (
                      <LockKeyhole size={16} className="text-[#c99bff]" />
                    )}
                  </div>
                </article>
              );
