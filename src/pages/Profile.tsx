    ...
    <main className="min-h-screen bg-[#120b20] px-5 py-8 text-[#fff8f2] sm:px-8 lg:px-12">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-8 flex items-center gap-4">
          ...
          <div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-[#c99bff]">Account settings</p><h1 className="font-display text-3xl font-black">Your profile</h1></div>
        </div>

        <section className="mt-8 rounded-[28px] border border-white/10 bg-[#211230] p-5 sm:p-7">
          <h2 className="font-display text-xl font-black">Personal details</h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <label className="text-sm font-bold">Name<input value={profile.display_name} onChange={(event) => setProfile({ ...profile, display_name: event.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-[#140b20] px-4 py-3 outline-none focus:border-[#a97cff]" /></label>
          ...
          <button onClick={() => void saveProfile()} disabled={isSaving} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#7c3aed] px-5 py-3 text-sm font-black disabled:opacity-50"><Save size={16} />{isSaving ? "Saving…" : "Save details"}</button>
        </section>
