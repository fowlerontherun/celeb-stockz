...
export default function MarketOperations() {
  ...
  const saveSettings = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      showSuccess("Data service settings saved.");
  ...
  const metricCards = [
    ...
    [
      "Flagged movements",
      String(data.metrics.flaggedSnapshots),
      "Held for review",
    ],
    [
      "Completed trades",
    ...
    [
      "Open orders",
      String(data.metrics.openOrders),
      "Evaluated after verified snapshots",
    ],
  ];
    ...
    <main className="min-h-screen bg-[#120b20] px-5 py-8 text-[#fff8f2] sm:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#c4b4d0]">
              Configure data providers, test live signals, control practice
              trading, and monitor the health of the market refresh system.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <Zap
                size={16}
                className={isTriggeringRefresh ? "animate-spin" : ""}
              />
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-[#e4d3e2]">
                {data.system.tradingPaused
                  ? "Market buys, sells, and new orders are blocked."
        ...
        <section className="mt-7 rounded-[28px] border border-[#62e7b6]/30 bg-[#162725] p-5 sm:p-7">
          ...
          <p className="mt-2 text-sm leading-6 text-[#c4ded8]">
            Check Wikipedia, GDELT, Google Search, YouTube, and practice-trade
            signals for a selected celebrity.
          </p>

          <div className="mt-5 flex flex-wrap items-center gap-3">
            <select
              value={selectedTestTicker}
              onChange={(event) => setSelectedTestTicker(event.target.value)}
            ...
            >
              {testableCelebrities.map((celebrity) => (
                <option key={celebrity.ticker} value={celebrity.ticker}>
                  {celebrity.name} (${celebrity.ticker})
                </option>
              ))}
            </select>
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-[#62e7b6] px-5 py-3 text-sm font-black text-[#112b24] transition hover:bg-[#80ecc8] disabled:opacity-50"
            >
              <Zap
                size={16}
                className={isTestingSignals ? "animate-spin" : ""}
              />
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-[#0f1b19] p-5">
              <div className="flex flex-wrap items-start justify-between gap-3 border-b border-white/10 pb-3">
                <div>
                  <h3 className="font-display text-lg font-black">
                    {testResult.name} (${testResult.ticker})
                  </h3>
                  <p className="text-xs text-[#a4c9c0]">
                    Wikipedia:{" "}
                    <span className="font-mono">
                      {testResult.wikipediaTitle}
                    </span>
                  </p>
                </div>
                <span
                  className={`rounded-lg px-2 py-1 text-xs font-black ${
                    testResult.channelMapped
                      ? "bg-[#183b33] text-[#62e7b6]"
                      : "bg-white/5 text-[#a4c9c0]"
                  }`}
                >
                  {testResult.channelMapped
                    ? `YouTube mapped (${testResult.channelId})`
                    : "No YouTube mapping"}
                </span>
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <DiagnosticCard
                  ...
                  value={
                    testResult.diagnostics.wikipedia.dailyPageviews === null
                      ? "—"
                      : testResult.diagnostics.wikipedia.dailyPageviews.toLocaleString()
                  }
                  detail="Yesterday's pageviews"
                  status={testResult.diagnostics.wikipedia.status}
                />
                <DiagnosticCard
                  icon={<Globe size={14} />}
                  label="Article edits"
                  value={
                    testResult.diagnostics.wikipedia.recentRevisions7d === null
                      ? "—"
                      : String(
                          testResult.diagnostics.wikipedia.recentRevisions7d,
                        )
                  }
                  detail="Revisions in the last seven days"
                  status={
                    testResult.diagnostics.wikipedia.revisionsStatus
                  }
                />
                <DiagnosticCard
                  ...
                  label="GDELT news"
                  value={
                    testResult.diagnostics.news.mentions7d === null
                      ? "—"
                      : testResult.diagnostics.news.mentions7d.toLocaleString()
                  }
                  detail="Global media mentions"
                  status={testResult.diagnostics.news.status}
                />
                <DiagnosticCard
                  icon={<Search size={14} />}
                  label="Google Search"
                  value={
                    testResult.diagnostics.search.resultsCount === null
                      ? "API key required"
                      : testResult.diagnostics.search.resultsCount.toLocaleString()
                  }
                  detail="Indexed search results"
                  status={testResult.diagnostics.search.status}
                />
                <DiagnosticCard
                  icon={<Youtube size={14} />}
                  label="YouTube stats"
                  value={
                    testResult.diagnostics.youtube.subscribers === null
                      ? "Channel or key required"
                      : `${(
                          testResult.diagnostics.youtube.subscribers / 1_000_000
                        ).toFixed(1)}M subscribers`
                  }
                  detail={
                    testResult.diagnostics.youtube.views === null
                      ? "Official channel statistics"
                      : `${(
                          testResult.diagnostics.youtube.views / 1_000_000
                        ).toFixed(0)}M views`
                  }
                  status={testResult.diagnostics.youtube.status}
                />
                <div className="rounded-xl bg-white/[.04] p-3.5">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1.5 text-xs font-bold text-[#e6f4f1]">
                      <TrendingUp size={14} className="text-[#c99bff]" />
                      Practice trades
                    </span>
                    <span className="text-[10px] font-black uppercase text-[#62e7b6]">
                      Verified
                    </span>
                  </div>
                  <p className="mt-2 text-xl font-black">
                    {testResult.diagnostics.trades.pressure >= 0 ? "+" : ""}
                    {testResult.diagnostics.trades.pressure.toFixed(3)}
                  </p>
                  <p className="mt-0.5 text-[11px] text-[#8ea8a2]">
                    24-hour STKZ order pressure
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="mt-7 rounded-[28px] border border-[#c99bff]/30 bg-[#211230] p-5 sm:p-7">
          ...
          <p className="mt-2 text-sm leading-6 text-[#c4b4d0]">
            These values are stored server-side and used by the scheduled
            market refresh.
          </p>
          ...
          <form onSubmit={saveSettings} className="mt-6 space-y-5">
            <div className="rounded-2xl border border-white/10 bg-[#160c25] p-5">
              ...
              <label className="mt-4 block text-xs font-bold uppercase tracking-[.12em] text-[#b9a9c5]">
                API key
                <input
                  type="password"
                  value={youtubeApiKey}
                  ...
                  placeholder="AIzaSy…"
                  className="mt-2 w-full rounded-xl border border-white/10 bg-[#211230] px-4 py-3 text-sm text-white outline-none focus:border-[#a97cff]"
                />
              </label>
              <label className="mt-4 block text-xs font-bold uppercase tracking-[.12em] text-[#b9a9c5]">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setYoutubeChannelsJson(event.target.value)
                  }
                  placeholder={'{\n  "TSWIFT": "UC..."\n}'}
                  className="mt-2 w-full rounded-xl border border-white/10 bg-[#211230] p-3 font-mono text-xs text-[#e6d8ff] outline-none focus:border-[#a97cff]"
                />
              </label>
            </div>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <label className="text-xs font-bold uppercase tracking-[.12em] text-[#b9a9c5]">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setMarketRefreshSecret(event.target.value)
                    }
            ...
            <div className="flex justify-end">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-[#7c3aed] px-6 py-3.5 text-sm font-black text-white transition hover:bg-[#8f57f6] disabled:opacity-50"
              >
                <Save size={16} />
  ...
  return (
    <div className="rounded-xl bg-white/[.04] p-3.5">
      <div className="flex items-center justify-between gap-2">
        <span className="flex items-center gap-1.5 text-xs font-bold text-[#e6f4f1]">
          <span className={diagnosticStatusClass(status)}>{icon}</span>
          {label}
        </span>
        <span
          className={`text-[10px] font-black uppercase ${diagnosticStatusClass(
            status,
          )}`}
        >
          {status}
      ...
      <p className="mt-0.5 text-[11px] text-[#8ea8a2]">{detail}</p>
    </div>
  );
}