...
export function WeeklyRecap({ onStartTrading }: { onStartTrading: () => void }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!preferencesResponse.ok || !recapResponse.ok) {
        throw new Error("Could not load your live progress.");
      }
  ...
  useEffect(() => {
    ...
    const milestones = [
      [recap.weeklyTradeCount > 0, "first-trade", "First live trade complete — nice start!"],
    ...
    <section className="mt-7 grid gap-4 xl:grid-cols-[1.35fr_.85fr]">
      {!preferences.onboarding_dismissed && preferences.education_tips && (
        <article className="rounded-[26px] border border-[#c99bff]/30 bg-[#291845] p-5 sm:p-6">
          ...
          <h2 className="font-display mt-2 text-2xl font-black">Make your first live move.</h2>
          <p className="mt-2 max-w-xl text-sm leading-6 text-[#d0bedc]">
            Browse a culture category, choose a market, and enter a STKZ amount. Every price is modeled for live trading, never real investing.
          </p>
          <button
            type="button"
      ...
      <article className="rounded-[26px] border border-white/10 bg-[#1e112f] p-5 xl:col-span-2">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[.17em] text-[#c99bff]"><Bell size={15} /> Live notifications</p><p className="mt-1 text-sm text-[#b9a9c5]">Control alerts and education without losing access to your account.</p></div>
